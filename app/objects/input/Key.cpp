...
namespace vks_engine
{
    ...
    Key::Key(): m_Key(0), m_Action(0), m_Mod(0)
    {
    }
    ...
    Key::Key(int key, int action, int mod): m_Key(key), m_Action(action), m_Mod(mod)
    {
    }
    ...
    bool Key::operator==(const Key &key) const
    {
        return m_Key == key.m_Key && m_Action == key.m_Action && m_Mod == key.m_Mod;
    }
}
